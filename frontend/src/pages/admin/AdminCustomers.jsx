...
export default function AdminCustomers() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      api
        .get("/admin/users", { params: { search, page, limit: 20 } })
        .then((r) => r.data),
    staleTime: 30000,
  });
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {users.map((u) => (
                      ...
                      >
                        ...
                        <td className="py-3 pr-3 font-semibold text-xs text-gray-900">
                          {u.totalOrders || 0}
                        </td>
                        <td className="py-3 pr-3 font-semibold text-xs text-brand-800">
                        ...
                        <td className="py-3 pr-3">
                          <span
                            className={`badge text-[10px] ${u.isActive ? "bg-green-100 text-green-700" : "bg-red-100 text-red-600"}`}
                          >
                        ...
                        <td className="py-3">
                          <button
                            onClick={() => {
                              if (
                                confirm(
                                  `${u.isActive ? "Ban" : "Activate"} ${u.name}?`,
                                )
                              )
                                toggleMutation.mutate(u._id);
                            }}
