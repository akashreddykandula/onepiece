  ...
  return (
    ...
    >
      <div className="flex items-start justify-between mb-4">
        ...
        {trend !== undefined && (
          <span
            className={`text-xs font-semibold px-2 py-1 rounded-full ${trend >= 0 ? "bg-green-100 text-green-700" : "bg-red-100 text-red-600"}`}
          >
            {trend >= 0 ? "+" : ""}
            {trend}%
          </span>
      ...
      <p className="font-display font-black text-3xl text-brand-900 mb-0.5">
        {value}
      </p>
      ...
      {linkTo && (
        <Link
          to={linkTo}
          className="inline-flex items-center gap-1 text-xs text-brand-600 font-medium mt-3 hover:text-brand-800 transition-colors"
        >
...
export default function AdminDashboard() {
  ...
  const statCards = [
    ...
    },
    {
      ...
      sub: `${stats.onlineUsers || 0} online`,
      icon: FiUsers,
      color: "bg-green-500",
  ...
  return (
    <>
      <Helmet>
        <title>Dashboard | ONE PIECE Admin</title>
      </Helmet>
          ...
          <div className="flex gap-3">
            {stats.pendingOrders > 0 && (
              <Link
                ...
                className="flex items-center gap-2 bg-amber-50 text-amber-700 border border-amber-200 px-4 py-2 rounded-xl text-sm font-medium hover:bg-amber-100 transition-colors"
              >
            ...
            {stats.returnsCount > 0 && (
              <Link
                to="/admin/returns"
                className="flex items-center gap-2 bg-red-50 text-red-600 border border-red-200 px-4 py-2 rounded-xl text-sm font-medium hover:bg-red-100 transition-colors"
              >
                  ...
                  <tr className="border-b border-gray-100">
                    {["Order", "Customer", "Amount", "Status", "Date"].map(
                      (h) => (
                        <th
                          key={h}
                          className="text-left text-xs text-gray-400 font-semibold uppercase tracking-wide pb-3 pr-4"
                        >
                          {h}
                        </th>
                      ),
                      ...
                      >
                        <td className="py-3 pr-4">
                          <Link
                            ...
                            className="font-medium text-brand-700 hover:text-brand-900 font-mono text-xs"
                          >
                            #{order.orderNumber}
                          </Link>
                        </td>
            ...
            <div className="space-y-4">
              {topProducts.map((product, i) => (
                <div key={product._id} className="flex items-center gap-3">
                  ...
                  <p className="text-xs font-bold text-brand-800 shrink-0">
