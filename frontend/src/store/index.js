...
export const updateProfileThunk = createAsyncThunk(
  ...
  async (d, { rejectWithValue }) => {
    try {
      const r = await authAPI.updateProfile(d);
      return r.data.user;
    } catch (e) {
      return rejectWithValue(e.response?.data?.message);
    }
...
export const toggleWishlistThunk = createAsyncThunk(
  ...
  async (productId, { rejectWithValue }) => {
    try {
      const r = await authAPI.toggleWishlist(productId);
      return r.data;
    } catch (e) {
      return rejectWithValue(e.response?.data?.message);
    }
...
const rvSlice = createSlice({
  ...
  reducers: {
    addToRecentlyViewed: (state, { payload }) => {
      state.items = [
        payload,
        ...state.items.filter((p) => p._id !== payload._id),
      ].slice(0, 12);
