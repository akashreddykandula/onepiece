...
exports.register = async (req, res, next) => {
  ...
  emailService.sendWelcome(user).catch(e => console.error('Welcome email:', e.message));
...
exports.login = async (req, res, next) => {
  ...
  if (!user.isActive) {
    return next(new AppError('Account deactivated. Contact support@onepiece.in.', 401));
  }
...
exports.getMe = async (req, res, next) => {
  const user = await User.findById(req.user._id).populate('wishlist', 'name images price comparePrice slug isInStock');
...
exports.forgotPassword = async (req, res, next) => {
  ...
  emailService.sendPasswordReset(user, resetUrl).catch(e => console.error('Reset email error:', e.message));
...
exports.addRecentlyViewed = async (req, res) => {
  ...
  await User.findByIdAndUpdate(req.user._id, {
    $push: { recentlyViewed: { $each: [req.params.productId], $position: 0, $slice: 20 } },
  });
